...
grounding: "this repo's own CLAUDE.md, which is unusually dense with falsifiable claims — \"rrt-doctor's own hook manifest pins it to stages: [manual]\", the six-defect table, the tool-grant anomaly note — exactly the shape of claim befund-docs-drift is built to check and that claude-md-improver's currency criterion only grades impressionistically."
dos:
...
grades currency on a letter scale, not claim by claim. befund-docs-drift closes that
