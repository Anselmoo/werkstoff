...
beats:
  ...
  - skill: "zirkel:zirkel-draft-revise"
...
grounding: "applying a reviewer's finding to `docs/plugin-authoring/references/craft-standards.md` is the document case where `zirkel-draft-revise` genuinely applies; the same finding applied to a plugin's `SKILL.md` behavior is a code change and belongs in the review entry above."
dos:
...
donts:
  ...
  - "Don't reach for zirkel-draft-revise on a code change -- it's scoped to prose, not code."
...
from what is preference, and closing the loop. `zirkel:zirkel-draft-revise` is the
