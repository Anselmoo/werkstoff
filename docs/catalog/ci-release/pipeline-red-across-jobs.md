...
grounding: "`.github/workflows/plugin-checks.yml` runs twenty-six checks with `continue-on-error: true` and collapses them into a single \"Fail the job if any check failed\" step. One red job can therefore mean any of twenty-six independent causes, which is exactly the shape this entry exists to untangle."
dos:
