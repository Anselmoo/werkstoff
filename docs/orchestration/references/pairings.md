...
pairings:
  ...
  - id: "business-rules-extractor-befund-extract-rules"
    skillA: "code-modernization:business-rules-extractor"
    skillB: "befund:befund-extract-rules"
  ...
  - id: "version-delta-analyst-befund-code-idiom"
    skillA: "code-modernization:version-delta-analyst"
    skillB: "befund:befund-code-idiom"
    source: "claude-plugins-official"
    beat: "inspect"
    why: "A same-stack uplift preserves code and tweaks it; version-delta-analyst finds which breaking changes actually bite this codebase, and befund-code-idiom then judges the result against the version the repo actually targets -- not the newest one."
    how: "version-delta-analyst is dispatched directly, standalone, sanctioned by modernize-brief.md itself rather than through the eight-stage pipeline: when the delta catalog is missing, spawn the version-delta-analyst agent directly, then return -- do not guess at the deltas. befund-code-idiom then runs against the manifest-declared version, and befund-idiom-fix applies only the mechanical fixes afterward."
    ...
    donts:
      - "Don't reach for befund-code-idiom to plan the uplift itself -- routing.md: it judges idioms against the language version the repo's manifest already declares, so it cannot plan a move to a version the manifest has not reached."
  ...
  - id: "zirkel-investigate-dynamically-systematic-debugging"
    skillA: "zirkel:zirkel-investigate-dynamically"
    ...
    how: "zirkel-investigate-dynamically finds where the behavior actually lives first. superpowers:systematic-debugging then works the located behavior to root cause, mandatory before any fix is proposed."
  ...
  - id: "zirkel-negotiate-tradeoffs-writing-plans"
    skillA: "zirkel:zirkel-negotiate-tradeoffs"
    ...
    how: "zirkel-explore-branches scores genuinely distinct readings of the request, zirkel-negotiate-tradeoffs settles any fork those branches can't resolve on their own, and superpowers:writing-plans turns the settled scope into a written plan with steps before anyone touches code."
    ...
    grounding: "\"make the plugins consistent\" splits on a documented boundary: plugins/passung/README.md lines 28-47, headed \"Scope -- read this before installing both this and befund\", route two categories of finding out to befund by name."
    recipeTask: "Scope an ambiguous task"
    recipeUrl: "/catalog/before-any-code/scope-ambiguous-task"

  - id: "zirkel-decompose-chain-systematic-debugging"
    skillA: "zirkel:zirkel-decompose-chain"
    skillB: "superpowers:systematic-debugging"
    source: "superpowers"
    beat: "split"
  ...
  - id: "frontend-design-befund-ui-audit"
    skillA: "frontend-design:frontend-design"
    skillB: "befund:befund-ui-audit"
    source: "claude-plugins-official"
    beat: "verify"
    ...
    how: "frontend-design implements the settled design first. befund-ui-audit then statically audits the built surface -- JSX/TSX, Vue/Svelte, HTML, CSS/SCSS -- for accessibility and hardcoded values, without ever running the app."
    ...
    donts:
      - "Don't run befund-ui-audit against a design that's still principles-only -- there's no markup yet to check."
      - "Don't substitute a live-app check for the static read; the skill's own scope is explicitly static-only."
    grounding: "the andon board viewer (plugins/andon/scripts/build_board_html.py) and the branch-comparison viewer (plugins/zirkel/scripts/build_branch_comparison_html.py), checked against tools/design-tokens/tokens.css."
    recipeTask: "Do UI and design-system work"
    recipeUrl: "/catalog/surface/ui-and-design-system-work"

  - id: "code-simplifier-passung"
    skillA: "pr-review-toolkit:code-simplifier"
    skillB: "passung:pattern-extractor"
    source: "claude-plugins-official"
    beat: "verify"
    ...
    how: "befund audits the documented half first (befund-lint-audit, then convention-auditor). passung:pattern-extractor then handles the undocumented half, refusing to force a pick when competing variants are genuinely tied, and passung-critic re-derives the verdict as a second judge. pr-review-toolkit:code-simplifier runs last, deliberately, on the now-aligned code only."
    ...
    dos:
      ...
      - "Let passung-critic re-derive pattern-extractor's verdict rather than rubber-stamping it."
    donts:
      ...
      - "Don't blur the documented/undocumented boundary -- passung's own README routes documented conventions out to befund by name; running both for the same finding double-reports it."
    grounding: "One of orchestration/README.md's three worked examples; plugins/passung/README.md's own \"Scope\" section is what draws the boundary."
  ...
  - id: "befund-docs-drift-claude-md-improver"
    skillA: "befund:befund-docs-drift"
    ...
    how: "befund-docs-drift reads CLAUDE.md by name among its four source files and verifies every falsifiable claim against the cited code by static comparison first. claude-md-improver then grades coverage, architecture clarity, and conciseness and proposes targeted diffs -- against a file whose claims are now known-accurate, not merely plausible."
    ...
    grounding: "this repo's own CLAUDE.md, dense with falsifiable claims -- \"rrt-doctor's own hook manifest pins it to stages: [manual]\", the six-defect table, the open befund:arch-health-auditor tool-grant anomaly -- exactly what befund-docs-drift is built to check."
    recipeTask: "Improve a stale CLAUDE.md without trusting its own claims"
    recipeUrl: "/catalog/surface/improve-a-stale-claude-md"

  - id: "befund-lint-audit-hookify"
    skillA: "befund:befund-lint-audit"
    skillB: "hookify:hookify"
    source: "claude-plugins-official"
    beat: "verify"
    why: "befund-lint-audit is explicitly read-only and never auto-fixes a violation it confirms, which leaves nothing standing guard against the next repeat -- and a plugin-wide, code-reviewed hook is the wrong weight class for one contributor's own habit."
    how: "befund-lint-audit extracts discrete rules from CLAUDE.md (this repo has no house-rules.md, so every rule is labeled \"CLAUDE.md (best-effort)\") and dispatches convention-auditor to confirm real violations against the actual code. hookify then turns one confirmed, repeatedly-broken rule into a live `.claude/hookify.<name>.local.md` guard, active on the next tool use with no restart."
    ...
    dos:
      - "Confirm the violation with befund-lint-audit's Find+Verify before authoring a guard -- hookify's own rule format has no way to check whether a pattern match is a real violation, only to react once one's typed."
  ...
  - id: "befund-portfolio-project-artifact"
    skillA: "befund:befund-portfolio"
    skillB: "project-artifact:project-artifact"
    source: "claude-plugins-official"
    beat: "verify"
    why: "befund-portfolio's own output is deliberately narrow -- one grade per repo, worst-signal-wins, written once to a local file with no sharing and no memory of the previous sweep."
    how: "befund-portfolio grades every repo in an explicit portfolio directory Red/Amber/Green/Gray and writes befund-portfolio.html into that directory. project-artifact then turns those verdicts into a living claude.ai page -- status pills, an Attention tab for what's blocked, and a delta-only refresh that reads the previous render's embedded state block."
    prompt: "sweep every repo under ~/LocalDocuments/GitHub_Forks and grade each one's befund health, then publish that as a shareable status page I can send the team"
    dos:
      - "Name the portfolio directory explicitly when invoking befund-portfolio -- its own gate refuses to infer cwd's parent as the portfolio."
    ...
    donts:
      - "Don't republish the raw befund-portfolio.html as-is -- it has no sharing, no tabs, and no delta tracking across sweeps."
      - "Don't let a Gray verdict get smoothed into an invented grade on the status page -- Gray means \"not yet assessed,\" nothing more."
    grounding: "the user's own ~/LocalDocuments/GitHub_Forks directory holds roughly 150 git repositories, including werkstoff itself -- exactly the \"explicit portfolio directory\" befund-portfolio's Step 1 scope gate requires."
  ...
  - id: "befund-docs-drift-comment-analyzer"
    skillA: "befund:befund-docs-drift"
    ...
    how: "befund-docs-drift verifies every extracted, in-scope claim against the current codebase, run immediately after a change while the diff is still legible. pr-review-toolkit:comment-analyzer separately checks the comments in the same touched files, since a docs sweep doesn't read those."
    ...
    donts:
      ...
      - "Don't run a convention survey (passung:pattern-analyst) until the docs are already accurate -- a survey run before the content is correct clusters variants of the wrong text."
