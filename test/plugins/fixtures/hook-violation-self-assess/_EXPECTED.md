# hook-violation fixture for befund

`guard_target_edit.py` is inert unless a befund remediator dispatch has
an edit-scope lock open (`analysis/befund/edit_scope.json`) -- a repo
merely having `.claude/befund.local.md` present with neither
...
befund-managed at all).

This fixture instead has an edit-scope lock open naming `src/api.py` under
`mode: "idiom_fix"`, as if `befund-idiom-fix` had dispatched
`idiom-remediator` for it -- but `.claude/befund.local.md` never
