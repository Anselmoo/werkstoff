# hook-violation fixture for vorbild

`.design/` is present, so this is a vorbild-managed repository and the guard is live.
...
violation here and must be allowed: vorbild's guard denies only inside the design root,
...
contradict this one, so it is covered by `plugins/vorbild/hooks/test_vorbild_guard.py`
