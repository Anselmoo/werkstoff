...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/consistency_record.py" event --area $1 --stage brief --status closed
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/consistency_record.py" event --area $1 --stage brief --status halted --reason "<the specific reason>"
```
