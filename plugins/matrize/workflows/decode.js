export const meta = {
  name: 'matrize-decode-references',
  description:
  ...
  whenToUse:
    'Invoked by matrize-decode when the Workflow tool is available. Requires args {root, references:[{slug, reliability, rights}]}. Covers measurement and refereeing only — writing DECODE.md stays in the calling session, because no fan-out agent may hold Write while reference content is untrusted input.',
  phases: [
...
if (!refs.length) {
  return { error: 'no references — run matrize-collect first', cards: [], referee: [] }
}
...
do not manufacture cards to fill the batch.`,
  ...
  if (remaining.length && measured === 0) {
    aborted = true
    abortReason = `no reference in batch ${batchNum} could be read (${batch.length} attempted). This is an acquisition problem, not a rubric problem: re-run matrize-collect and check each PROVENANCE.md source before spending more.`
