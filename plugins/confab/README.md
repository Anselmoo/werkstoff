...
- **`confab-cycle`** — Bounded autonomous self-optimization cycle:
  ...
  via `confab-remediator` — for `dependency_audit`/`contract_drift`,
  same-file findings are clustered into a single dispatch that applies
...
- **`confab-remediator`** (`color: red`) — applies one scoped fix per
  already-verified finding, independently, for a same-file
  `dependency_audit`/`contract_drift` batch (or a single
  `agentic_reliability` `excessive-tool-grant` finding, always
  dispatched alone), then reports what changed per finding; `Read`/`Edit`
  only,
