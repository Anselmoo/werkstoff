...
Copy this file to `.claude/quality.local.md` in the repo you're
...
Note: `skip_verification` has no effect on `quality-assertion-audit` —
that skill's Verify phase always runs regardless of this setting, per
its own documented design (see `references/settings.md`).

Note: `cycle.mode: fix` lets `quality-cycle` edit files in this repo
