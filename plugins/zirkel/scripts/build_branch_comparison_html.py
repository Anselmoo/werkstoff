#!/usr/bin/env python3
"""Renders a zirkel-explore-branches run's persisted state.json (branches
...
Total and the winner are ALWAYS recomputed here via zirkel_lib's own
validate_branch_scores -- never read from a stored field, because no
"total"/"selected"/"winner" field is persisted in state.json's
explore.branches today (zirkel.py's state-write writes the caller's raw
...
This means the report can never disagree with what zirkel.py's guard would
...
import zirkel  # noqa: E402  -- validate_state, safe to import (no import-time side effects)
import zirkel_lib as C  # noqa: E402

DEFAULT_OUTPUT_DIR = C.DEFAULT_OUTPUT_DIR  # ".zirkel"


def load_state(repo_root: str, run_id: str) -> dict:
    """Read and gate-validate runs/<run_id>/state.json under DEFAULT_OUTPUT_DIR.
    Raises FileNotFoundError for an unknown run, or zirkel_lib.GuardError for
    one that fails the same validation zirkel.py's own state-write/state-read
...
def find_latest_explore_run(repo_root: str) -> str | None:
    ...
    rather than fatal -- same tolerance zirkel.py's own state-find uses."""
    ...
    if os.path.isdir(runs_root):
        for name in sorted(os.listdir(runs_root)):
            ...
            try:
                with open(state_path, encoding="utf-8") as fh:
                    state = json.load(fh)
                zirkel.validate_state(state, f"state({name})")
...
def build_payload(state: dict) -> dict:
    """Recompute Total + winner via zirkel_lib's own guard (never trust a
    ...
    it, since zirkel.py's guard only ever needed the 3 score axes."""
...
def main(argv=None):
    ...
    parser.add_argument("--out", help="defaults to .zirkel/runs/<run-id>/branch-comparison.html")
    ...
    if args.out:
        ...
        # enforces. When --out is omitted, this script cannot write outside .zirkel/.
