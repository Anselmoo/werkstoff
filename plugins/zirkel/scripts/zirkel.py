#!/usr/bin/env python3
"""zirkel.py — the guard CLI every zirkel skill invokes.

Usage:
    python3 zirkel.py <check> [--in PAYLOAD.json | -]  [extra args]

Each <check> reads a JSON payload (from a file, or '-' for stdin), runs the
corresponding enforcement function in zirkel_lib, and:
...
matches a given task byte-for-byte, so zirkel-solve's Clarify/Explore steps
...
def _cli_state_find(p, a):
    ...
    modified match — this is what lets zirkel-solve's Clarify/Explore
    steps reuse a prior standalone zirkel-clarify-scope/zirkel-explore-
...
def _cli_run_event(p, a):
    """Append one phase boundary or halt to .zirkel/runs/<run_id>/run.jsonl.

    zirkel-solve used to write state.json once, at the end, so a run that paused
    ...
    try:
        ...
        # `runs/../x`, which still lands inside .zirkel/ but outside runs/.
        run = run_record.open_run("zirkel", run_id, directory=run_record.Path(os.path.dirname(safe)))
        if status == "halted":
...
def main(argv: list[str]) -> int:
    if not argv or argv[0] in ("-h", "--help"):
        sys.stderr.write(
            "usage: zirkel.py <check> [--in PAYLOAD.json | -] [args]\n"
