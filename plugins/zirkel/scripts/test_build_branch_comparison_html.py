...
ok("build_payload breaks ties by lower risk (via zirkel_lib.validate_branch_scores)", payload_tie["selected"] == "Y")
ok("build_payload defaults a missing description to empty string, not a crash", payload_tie["branches"][0]["description"] == "")

# --- load_state / find_latest_explore_run against a real .zirkel/runs/ tree ---
def _write_state(tmp_repo, run_id, state):
    run_dir = os.path.join(tmp_repo, ".zirkel", "runs", run_id)
...
try:
    ...
    out_path = os.path.join(_tmp2, ".zirkel", "runs", _fixture_state["run_id"], "branch-comparison.html")
    ok("main() writes the report to the default .zirkel/runs/<id>/ path", os.path.isfile(out_path))
    ...
    ok("rendered report carries the standard <title> shape",
       "<title>zirkel \u2014 branch comparison</title>" in html)
