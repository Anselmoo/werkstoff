...
// of the like-named zirkel-* skills/workflows (this script only ever makes one generic agent()
...
function normalizeArgs(a) {
  ...
  throw new Error(
    'zirkel-solve: args arrived as a string that is not a JSON object — ' +
...
if (!rawTask) throw new Error('zirkel-solve: args.task (the raw task) is required')
const phasesRun = []

// ---------- CLARIFY ----------
// Workflow scripts have no filesystem access, so this script cannot call
// `zirkel.py state-find` itself. When the calling SKILL.md already did that
// lookup (rule: solve-reuses-prior-standalone-run) and found a prior
// zirkel-clarify-scope run for this exact `rawTask`, it passes the result in
...
if (blocking.length > 0) {
  ...
  return {
    ...
    next: 'Resolve each blocking uncertainty (zirkel-verify-assumptions), then re-run with answers.',
  }
}

// ---------- EXPLORE (conditional) ----------
// Same filesystem constraint as Clarify above: if the calling SKILL.md found a
// prior zirkel-explore-branches run matching clarify.scoped_task, it passes
...
if (hasStrategicFork) {
  ...
  if (parsedArgs?.priorExplore) {
    ...
    log(`Explore: reusing a prior zirkel-explore-branches run (args.priorExplore). Selected: ${explore.selected}`)
  } else {
    explore = await workflow('zirkel-explore-branches', { problem: clarify.scoped_task,
