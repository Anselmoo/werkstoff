...
description: >-
  ...
  "explore alternatives", or the Explore phase of zirkel-solve.
---
...
Before dispatching any `branch-proposer`, build (or reuse) zirkel's symbol-index
...
## Persist (so a later `zirkel-solve` can reuse this, and so Render below has
something to render)
...
A later `zirkel-solve` run only reuses this if its own Explore step is about to
score branches for that **exact same text** (see `zirkel-solve`'s own "Reuse a
...
`.zirkel/runs/<run-id>/branch-comparison.html`. Mention this path when
