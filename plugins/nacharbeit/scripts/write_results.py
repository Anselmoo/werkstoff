#!/usr/bin/env python3
"""Persist a nacharbeit review workflow return value — or refuse, loudly.
...
<state>/{run,findings,routing,synthesis}.json, and it writes them only when the return
value is complete, calibrated, and stamped for the args that produced it. Anything else
leaves a FAILED-<stamp> marker and nothing more, so build_report.py cannot render a
stale or partial run as a fresh one (CLAUDE.md: "a failed run leaves the previous output
in place").

Usage: write_results.py <return.json> [--state-dir analysis/nacharbeit] [--args <state>/args.json]
...
def refuse(reason: str, stamp: str) -> int:
    OUT.mkdir(parents=True, exist_ok=True)  # noqa: F821 -- module global, rebound in main()
...
def main(argv: list[str] | None = None) -> int:
    global OUT
    ap = argparse.ArgumentParser(description="Persist a completed nacharbeit review, or refuse.")
    ap.add_argument("ret", type=Path)
    ap.add_argument("--state-dir", type=Path, default=None)
    ap.add_argument("--args", type=Path, default=None, help="args JSON (default <state-dir>/args.json)")
    a = ap.parse_args(argv)
    OUT = nc.state_dir(a.state_dir)
    if a.args is None:
        a.args = OUT / "args.json"
    ...
    if not isinstance(ret["calibration"], dict) or not ret["calibration"].get("rounds"):
        return refuse("calibration block is empty — the finder was never calibrated, so nothing here is a measurement", stamp)
