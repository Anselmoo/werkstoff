...
def default_fixtures_root() -> Path:
    """test/plugins/fixtures of the repo being run in. Searched from the working
    directory first: nacharbeit vendors a copy of this file under
    plugins/<name>/scripts/, where parents[2] is not the repo root."""
    cwd = Path.cwd() / "test/plugins/fixtures"
    return cwd if cwd.is_dir() else REPO_ROOT / "test/plugins/fixtures"


DEFAULT_VIOLATING = "ledger-missing-blast-radius"
...
def violating_fixture_for(plugin: Path, override: str) -> Path:
    ...
    specific = FIXTURES_ROOT / f"hook-violation-{plugin.name}"
    if specific.is_dir():
        return specific
    o = Path(override)
    return (o if o.is_absolute() or o.exists() else FIXTURES_ROOT / o).resolve()
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--violating-fixture", default=DEFAULT_VIOLATING,
                    help="a repo state the hook is supposed to refuse edits in (a name under the fixtures root, or a path)")
    ap.add_argument("--fixtures-root", type=Path, default=None,
                    help=f"where hook-violation-<plugin>/ fixtures live (default {FIXTURES_ROOT})")
