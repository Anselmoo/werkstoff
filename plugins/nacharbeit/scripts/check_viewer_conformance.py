...
Usage: check_viewer_conformance.py [--plugin-dir DIR ...]
Exit: 0 conformant (or no viewer in the named dirs); 1 any violation.
"""
...
def check(plugin: str, path: str, plugin_dir: str | None = None) -> list[str]:
    """Violations for one viewer. `plugin_dir` lets a caller outside this repo
    (nacharbeit's vendored copy) point at the plugin the viewer belongs to; the
    README and the relative paths in messages are then derived from it instead
    of from REPO/plugins/<plugin>."""
    base = os.path.dirname(os.path.dirname(os.path.abspath(plugin_dir))) if plugin_dir else REPO
    ...
    if not os.path.isfile(shot):
        errs.append(f"{os.path.relpath(shot, base)}: C1 -- screenshot missing")
    else:
...
def viewers_in(plugin_dir: str) -> list[tuple[str, str]]:
    """(plugin, viewer path) for one plugin directory, by its directory name."""
    plugin = os.path.basename(os.path.abspath(plugin_dir))
    assets = os.path.join(plugin_dir, "assets")
    if not os.path.isdir(assets):
        return []
    return [(plugin, os.path.join(assets, n)) for n in sorted(os.listdir(assets)) if n.endswith("-viewer.html")]


def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(description="Check report viewers against report-viewer-standard.md.")
    ap.add_argument("--plugin-dir", action="append", default=[],
                    help="check only this plugin directory (repeatable); default: every plugins/*/")
    args = ap.parse_args(argv)
    if args.plugin_dir:
        found = [(p, v, d) for d in args.plugin_dir for p, v in viewers_in(d)]
        if not found:
            print("No report viewer in the given plugin directories; nothing to check.")
            return 0
    else:
        found = [(p, v, None) for p, v in viewers()]
        if not found:
            print("FAIL: no viewers matched plugins/*/assets/*-viewer.html", file=sys.stderr)
            return 1
    errors = []
    for plugin, path, plugin_dir in found:
        errors.extend(check(plugin, path, plugin_dir=plugin_dir))
    for err in errors:
        print(f"FAIL: {err}")
    if errors:
