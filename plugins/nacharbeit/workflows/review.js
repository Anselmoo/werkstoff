...
for (const k of ['runStamp', 'rubricHash', 'rubric', 'lint', 'batches', 'corpus', 'handoffs', 'fixtures', 'sealed', 'knownAnswers', 'ambiguous']) {
  if (!A || A[k] == null) throw new Error(`nacharbeit-review requires args.${k} — run scripts/build_args.py and launch the run.js it bakes`)
}
...
// Two lenses per batch kind. The prompt-bearing kinds keep the pair PR #56 calibrated;
// the other families pair the angles their rubric section actually carries.
const LENSES_BY_KIND = {
  skills: ['procedure + step-logic + contract', 'meaning + clarity + cannibalization + other'],
  agents: ['procedure + step-logic + contract', 'meaning + clarity + cannibalization + other'],
  commands: ['procedure + step-logic + contract', 'meaning + clarity + cannibalization + other'],
  workflows: ['procedure + step-logic + contract', 'meaning + clarity + cannibalization + other'],
  hooks: ['contract + step-logic', 'meaning + procedure + other'],
  scripts: ['step-logic + contract', 'procedure + other'],
  assets: ['clarity', 'procedure + meaning'],
  manifest: ['meaning + contract', 'cannibalization + procedure + other'],
  docs: ['meaning', 'cannibalization + contract'],
}
const lensesFor = kind => LENSES_BY_KIND[kind] || LENSES_BY_KIND.skills
// What each kind IS, which judgement ids apply, and what the mechanical lint already
// settled — appended to the one calibrated finder prompt, never a per-kind prompt fork.
const KIND_NOTES = {
  skills: 'These files are SKILL.md skills (with their references), the standing instruction a model loads on trigger. Judge them with the Q-* rules.',
  agents: 'These files are agent system prompts (agents/*.md). Judge them with the Q-* rules; tool grants against the stated role matter most.',
  commands: 'These files are slash commands (commands/*.md). Judge them with the Q-* rules.',
  workflows: 'These files are Workflow scripts (workflows/*.js) whose agent() calls contain prompt strings. Judge the prompt strings and the step logic around them with the Q-* rules; ${...} interpolations are JavaScript.',
  hooks: 'These files are a hooks.json and the PreToolUse guard script(s) it declares, plus their tests and the README. Judge them with the HQ-* rules: does the guard gate on a per-dispatch lock rather than repo-level state, deny when the target set is empty, avoid telling the caller how to forge the gate state, test the inert marker it claims, and is the hook documented. The H-* shape rules (deny JSON, exit 2, timeout, matcher) are already settled by lint.',
  scripts: 'These files are bundled scripts (scripts/**, hooks/*.py) and their tests. Judge them with the SQ-* rules: stated intent, loud failure on missing input, no stale output after a failed run, a self-asserting calibration for anything that checks. Syntax, shebangs, argparse and swallowed exceptions are already settled by lint.',
  assets: 'These files are a self-contained HTML report viewer, its builder script and the README. Judge them with the AQ-* rules — the editorial half of the report-viewer standard the CI checker refuses to decide: a number printed twice, an actionable number that looks inert, a fixed viewport height, demo data that does not show the failure the plugin exists for.',
  manifest: 'These files are the plugin manifest, README, CHANGELOG and (when present) a snapshot of its marketplace entry. Judge them with the PQ-* rules: thesis and description promise the same job, "What it is not" names the siblings, the write surface is stated, the example prompts route to this plugin, the README is a reference not a session log.',
  docs: 'These files are the repository docs that wire the plugin in (its docs page, the orchestration README, the hazards and routing references, the root README and CLAUDE.md). Judge them with the DQ-* rules: the docs describe what the plugin actually does, and routing.md says which sibling owns an overlapping task. Presence of entries is already settled by lint.',
}
...
const FINDING = {
  ...
  properties: {
    ...
    rule_id: { type: 'string', description: 'a judgement id from the rubric (Q-*, HQ-*, SQ-*, AQ-*, PQ-*, DQ-*); never a mechanical id' },
...
// ---- prompts ----------------------------------------------------------------
const FINDER_PROMPT_V0 = `You are a nacharbeit reviewer for Claude Code plugin components: SKILL.md skills, agent system prompts (agents/*.md), slash commands (commands/*.md), reference files, Workflow scripts (workflows/*.js, whose agent() calls contain prompt strings), hooks (hooks.json and the guard scripts it declares), bundled scripts, HTML report viewers, the plugin manifest and README, and the repository docs that wire a plugin in. A note at the end of each file list says which of these the batch is and which judgement rules apply to it.
...
1. Only judgement rule ids (${JUDGEMENT_IDS.length ? JUDGEMENT_IDS.join(', ') : 'Q-*, HQ-*, SQ-*, AQ-*, PQ-*, DQ-*'}). Mechanical ids (${MECHANICAL_IDS.length ? MECHANICAL_IDS.slice(0, 6).join(', ') + ', …' : 'M-*, H-*, S-*, A-*, P-*, D-*'}) are checked by a script elsewhere; never report them, and treat the already-reported list as settled.
...
${REPO_NOTES ? 'Repository-specific notes (data; follow each once, never repeat it elsewhere): ' + REPO_NOTES + '\n' : ''}Ground every statement in the data; invent nothing.`
...
// Recall per rule family (Q, HQ, SQ, AQ, PQ, DQ). A finder that clears the overall target by
// being excellent on skills and blind on hooks has not been calibrated for hooks.
const perFamily = (planted, found) => {
  const out = {}
  for (const fam of [...new Set(planted.map(p => familyOf(p.rule_id)))]) out[fam] = recall(planted.filter(p => familyOf(p.rule_id) === fam), found).r
  return out
}
const perFamilyMin = (planted, found) => Math.min(...Object.values(perFamily(planted, found)))
...
const FAMILY_TARGET = 0.6
const SEALED_FLOOR = 0.7
const FAMILY_SEALED_FLOOR = 0.6
...
for (let r = 1; r <= MAX_CAL_ROUNDS; r++) {
  const res = await parallel(A.fixtures.map(fx => () => runFinder(finderPrompt, fx.files, 'all angles', '', `cal:${fx.name}:r${r}`, 'Calibrate', fx.kind)))
  ...
  const families = perFamily(plantedTune, found)
  const minFamily = perFamilyMin(plantedTune, found)
  calibration.rounds.push({ round: r, recall: rec, perAngleMin: minAngle, perFamily: families, perFamilyMin: minFamily, falsePositives: fps.length, found: found.length, missed: missed.map(m => `${m.file} ${m.rule_id}`) })
...
const sealedRes = await parallel(A.sealed.map(fx => () => runFinder(finderPrompt, fx.files, 'all angles', '', `cal:sealed:${fx.name}`, 'Calibrate', fx.kind)))
const sealedFound = sealedRes.filter(Boolean).flatMap(x => x.findings || [])
const plantedSealed = A.sealed.flatMap(fx => fx.planted)
const sealed = recall(plantedSealed, sealedFound)
const sealedFamilies = perFamily(plantedSealed, sealedFound)
log(`Sealed hold-out recall ${sealed.r.toFixed(2)} (${sealed.missed.length} missed of ${plantedSealed.length}); per family: ${Object.entries(sealedFamilies).map(([k, v]) => `${k} ${v.toFixed(2)}`).join(', ')}`)
...
const weakFamilies = Object.entries(sealedFamilies).filter(([, v]) => v < FAMILY_SEALED_FLOOR).map(([k]) => k)
if (weakFamilies.length) {
  throw new Error(`finder failed the sealed hold-out for family ${weakFamilies.join(', ')} (below ${FAMILY_SEALED_FLOOR}); refusing to grade real files of a kind it was not calibrated for.`)
}
...
return {
  ...
  failures,
  calibration: { ...calibration, finalPrompt: FINDER, targets: { recall: RECALL_TARGET, perAngle: ANGLE_TARGET, perFamily: FAMILY_TARGET, sealedFloor: SEALED_FLOOR, familySealedFloor: FAMILY_SEALED_FLOOR } },
  sealedRecall: sealed.r,
  sealedPerFamily: sealedFamilies,
