...
A *Passung* is the fit: the tolerance class — H7/g6 and its siblings — that decides
whether two mating parts go together, and how. Two parts can each be within spec and
still not mate, because a fit is a property of the pair, not of either one alone.
That is the failure this plugin exists for: several variants of one convention, each
defensible on its own, that do not sit together. It picks the fit and works to it.
...
### Scope — read this before installing both this and `befund`
...
- A convention already written down somewhere (`CLAUDE.md`,
  ...
  a documented-convention auditor's job (e.g. `befund`'s
  `convention-auditor`).
- An idiom made obsolete by the language/framework version this codebase
  ...
  (e.g. `befund`'s `idiom-auditor` / `idiom-remediator`).
- What's left — N ≥ 2 variants, all still valid, none documented as the
  standard — is the entire reason `/passung-canonize` exists.
  `/passung-scan` actively routes the first two cases *out* rather
...
/plugin install passung@werkstoff
```
...
`/passung-preflight` to check all at once):
...
- **Real git history** — shallow or squashed history degrades every
  derived Pattern Card's confidence; `/passung-canonize` still runs,
  just on frequency alone.
- **A runnable test suite** — enables real equivalence proof in
  `/passung-verify`. Without one, verification degrades to a
  structural-diff-only review, and `/passung-preflight` reports
...
```bash
claude --plugin-dir /path/to/werkstoff/plugins/passung
```
...
> Triggers `passung-preflight` — read-only readiness report (stack detection,
...
> Triggers `passung-scan` — inventories undocumented, non-deprecated divergence per
...
> Triggers `passung-map` — renders the scan as a module × dimension heatmap
...
cp plugins/passung/scripts/testdata/sample_matrix.json \
    /tmp/cc-demo/analysis/billing/matrix.json
python3 plugins/passung/scripts/build_matrix_html.py /tmp/cc-demo billing \
    --template plugins/passung/assets/matrix-viewer.html \
    --d3 plugins/passung/assets/inline-d3.html \
    --tokens plugins/passung/assets/tokens.css
...
> Triggers `passung-canonize` — weighs frequency, git-history maturity, and adoption
...
> Triggers `passung-brief` — synthesizes discovery into a phased, dependency-first
...
> Triggers `passung-align` — applies the canon in place, one pilot module first, then
...
> Triggers `passung-verify` — test-suite equivalence (or structural-diff fallback)
...
> Triggers `passung-status` — read-only artifact inventory, staleness flags, and the
...
```bash
/passung-preflight billing      # is my environment ready?
/passung-scan billing           # what's actually inconsistent, and what's out of scope?
/passung-map billing            # show me the divergence matrix
```

Then the full path:

```bash
/passung-canonize billing                       # derive the canon per dimension, with provenance
/passung-brief billing                           # the plan a maintainer approves (HITL gate)
/passung-align billing error-handling-style      # apply it, in place, dependency-aware
/passung-verify billing error-handling-style      # prove nothing observable changed
/passung-status billing                          # where am I, what's stale, what's next
```
...
Keep Bash on a *prompted* permission mode during `/passung-align`'s
...
another agent's description; and `/passung-brief` is a human approval
...
- **`pattern-extractor`** — Weighs frequency/maturity/recency and decides
  ...
  Read-only. *(canonize)*
- **`passung-critic`** — Adversarial reviewer, skeptical of both
...
- **`/passung-preflight <area-dir>`** — Environment readiness check.
  ...
  baseline `/passung-verify` diffs against), inventories documented-
  convention sources (feeds `/passung-canonize` Step 1), checks git
  history depth (feeds maturity/recency weighting), and checks the scope
  boundary. Produces `PREFLIGHT.md`.

- **`/passung-scan <area-dir> [convention-pattern]`** — Inventories
...
- **`/passung-map <area-dir>`** — Renders the inventory as a
...
- **`/passung-canonize <area-dir> [dimension-pattern]`** — The
...
- **`/passung-brief <area-dir>`** — Synthesizes discovery into a
...
- **`/passung-align <area-dir> [dimension]`** — Applies the approved
...
- **`/passung-verify <area-dir> [dimension]`** — Proves an alignment
...
- **`/passung-status <area-dir>`** — Read-only progress report:
  artifact inventory, staleness flags, and the single most useful next
  command.

## What is enforced, and what is not

passung registers no `PreToolUse` hook, so no tool call is denied at the
...
- **`/passung-brief` is a human approval gate.** It reads the discovery artifacts,
...
- **`/passung-align`'s circuit breaker.** The batched fan-out runs one pilot module
...
- **`/passung-verify`'s second adversarial pass.** Every PASS verdict is re-derived
...
None of this is a `PreToolUse` denial — a model that skips `/passung-brief` and
...
```bash
python3 test/plugins/lint-frontmatter.py plugins/passung
claude plugin validate plugins/passung --strict
python3 plugins/nacharbeit/scripts/nacharbeit_lint.py plugins/passung --docs-root docs
python3 test/plugins/lint-release-wiring.py
node --check plugins/passung/workflows/scan.js
node --check plugins/passung/workflows/canonize.js
node --check plugins/passung/workflows/align.js
node --check plugins/passung/workflows/verify.js
