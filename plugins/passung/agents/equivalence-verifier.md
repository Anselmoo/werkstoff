...
description: Independently re-checks that an aligned module behaves identically to before alignment and that its docs/comments still match the code. Re-derives evidence from the diff and the module's tests rather than trusting the aligner's own report. Use as the verification pass after passung-align.
