...
4. **When it's genuinely close, say so.** If frequency, maturity, and
   ...
   wrong pick here becomes a mass-applied change in `/passung-align`;
...
5. **Every card states its basis in the open**, not just its conclusion —
   ...
   a human reviewing `/passung-brief` can disagree with your weighing
...
`/passung-canonize` defines, instead of the JSON object above.
