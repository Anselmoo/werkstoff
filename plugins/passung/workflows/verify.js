export const meta = {
  ...
  whenToUse:
    'Invoked by /passung-verify when the Workflow tool is available. Requires args {area, dimension, units: [{name, path}]} plus a prior align pass\'s analysis/<area>/ALIGN_NOTES.md, which the Check phase reads directly rather than trusting its summary. Covers the check + adversarial re-check only — writing VERIFICATION.md stays in the calling session.',
  phases: [
...
if (!area || !dimension || !Array.isArray(units) || units.length === 0) {
  throw new Error(
    'passung-verify-scan workflow requires args: {area, dimension, units: [{name, path}]} — e.g. {area:"billing", dimension:"error-handling-style", units:[{name:"billing-core", path:"billing/core"}]}',
  )
}
