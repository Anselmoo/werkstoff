export const meta = {
  name: 'passung-align-batch',
  description:
    'Batched fan-out of /passung-align\'s Step 2: one align-executor agent per unit, in dependency-aware escalating batches behind a per-batch circuit breaker',
  whenToUse:
    'Invoked by /passung-align ONLY after the pilot unit is aligned in-session, analysis/<area>/PLAYBOOK.md is written, and the human has approved the fan-out via an approved CONSISTENCY_BRIEF.md. Requires args {area, dimension, units: [{name, path, deps?}], batchSize?}. Each unit\'s optional `deps` lists sibling unit NAMES whose canonical form this unit\'s alignment depends on; a unit is only batched once every listed dep has ALIGNED. Agents write only inside their own unit directory — disjoint directories, so no worktree isolation is needed; shared files above unit level are owned by the calling session. Returns per-unit results plus three RE-PASSABLE unit lists ({name, path, deps}) — remainingUnits (never attempted), failedUnits (attempted, tests failed), blockedUnits (skipped because a dependency failed) — any of which can be passed straight back as the next invocation\'s `units`. The calling session applies returned sharedFileNeeds and folds playbookGaps into the playbook before re-invoking.',
  phases: [
    {
      title: 'Align',
      detail:
...
`
  ...
  const results = await parallel(
    batch.map(u => () =>
      agent(promptFor(u, gapsBlock), {
        agentType: 'passung:align-executor',
        label: `align:${u.name}`,
        phase: 'Align',
  ...
  if (remaining.length && measured.length === 0) {
    aborted = true
    abortReason = `no unit in batch ${batchNum} could run tests (testsRan:false on all ${batch.length}) — see results[].testCommand for why. This is an environment problem, NOT a playbook problem: a fan-out that cannot prove any unit aligned is spending money blind. Fix the test invocation, or — if this area genuinely has no per-unit tests — align the remaining units in-session and rely on /passung-verify's structural-diff-only mode instead of this fan-out.`
...
if (failedUnits.length && !aborted) {
  log(
    `${failedUnits.length} attempted unit(s) did not align — see results[].testFailures. They are NOT aligned and are returned in failedUnits (re-passable). Fold their playbookGaps into the playbook first; do not move to /passung-verify while any unit is unaligned.`,
  )
}
