---
description: Synthesize discovery into a phased Consistency Brief — the approved plan /passung-align executes against
...
so and stop — they come from `/passung-scan`, `/passung-map`, and
`/passung-canonize` respectively. Read `analysis/$1/PREFLIGHT.md` too,
...
wins and calibrate `/passung-align`'s batch behavior on low-risk
...
- Exit criteria (what `/passung-verify` must show)
...
Exit: `/passung-verify` shows all 14 sites raising `AppError`; existing tests pass
Scale: 14 sites
...
`/passung-align`'s batched fan-out runs on the rest. Say explicitly
...
State what `/passung-verify` will run per phase: full test suite,
targeted subset, or structural-diff-only (if `/passung-preflight`
...
```bash
python3 -B "${CLAUDE_PLUGIN_ROOT}/scripts/passung_record.py" event --area $1 --stage brief --status closed
```

If it stopped instead — the brief could not be approved, or an input artifact is missing — record that, with the specific reason, and do not
record `closed`:

```bash
python3 -B "${CLAUDE_PLUGIN_ROOT}/scripts/passung_record.py" event --area $1 --stage brief --status halted --reason "<the specific reason>"
```
