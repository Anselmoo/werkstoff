...
`/passung-brief $1` to produce and get one approved.
...
Never touch anything `/passung-preflight` Check 6 or Check 0 flagged as
...
- **Never edits outside its declared module/dimension scope**, even to fix
  ...
  a future `/passung-scan`, don't fold it in silently.
...
`/passung-verify` reads to know what to check.
...
single next command: `/passung-verify $1` once a phase is fully
