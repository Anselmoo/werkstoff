---
description: Use after /passung-scan to decide which variant of a divergent convention is canonical, with a provenance-tracked Pattern Card each
...
`passung` exists: the gap between "documented convention"
...
**Step 1 — Explicit source, one more time.** `/passung-scan` already
...
`/passung-preflight` already states: canonize falls back to
...
`/passung-brief` can see the actual basis, not just a score.
...
- Cards in full, ordered by blast radius (largest first — that's what a
  reviewer needs to see first in `/passung-brief`)
...
`/passung-align` executes against:
...
it can anchor `/passung-brief`. Tell the user the agent count before
...
```bash
python3 -B "${CLAUDE_PLUGIN_ROOT}/scripts/passung_record.py" event --area $1 --stage canonize --status closed
```
...
```bash
python3 -B "${CLAUDE_PLUGIN_ROOT}/scripts/passung_record.py" event --area $1 --stage canonize --status halted --reason "<the specific reason>"
```
