...
- A convention documented in `CLAUDE.md`, `house-rules.md`, a linter config,
  ...
  documented rule belongs to a conventions auditor (e.g. `befund`'s
...
- An idiom made obsolete by the language/framework version this codebase
  ...
  version-driven modernization (e.g. `befund`'s `idiom-auditor`).
  `passung` only cares about variants that are **all still
...
Adapt to the detected stack (Check 1 from `/passung-preflight`), but
...
```json
{
  ...
  "outOfScope": [
    ...
    { "id": "optional-typing", "reason": "deprecated for declared Python >=3.10 target — see befund idiom-auditor" }
  ]
}
```

This schema feeds `/passung-map`'s matrix and `/passung-canonize`'s
...
sites. Suggest `/passung-map $1` to see it visually, or
`/passung-canonize $1` to go straight to derivation.
