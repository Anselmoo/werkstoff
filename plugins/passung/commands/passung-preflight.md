...
be proven safe without a working test suite, and `/passung-canonize`
...
2. **Build & test locally** — Can this environment run the test suite and
   the linter/formatter locally? `/passung-verify` needs this to prove
...
4. **Off limits** — Is any subdirectory frozen, generated, vendored, or
   owned by another team and therefore not eligible for `/passung-align`
   even if it diverges?
...
and lets `/passung-canonize` route documentation lookups (e.g. Python
...
- Run the test suite (or a representative subset) and record pass/fail
  counts and wall-clock time. This is the baseline `/passung-verify`
...
- Run the linter/formatter in check-only mode (`--check`, `--dry-run`) and
  ...
  config, tool crash) is reported now, not discovered mid-`/passung-align`.

If the test suite cannot run at all (no test framework, broken environment),
say so plainly: `/passung-verify` will degrade to a structural diff
review with no executable proof, and `/passung-align` should stay
scoped to low-risk, mechanically-reversible changes (formatting, naming,
docstring shape) until that's fixed.

## Check 4 — Documented-convention sources (feeds `/passung-canonize` Step 1)
...
Report what was found and where. If a person-run `befund` (or
...
`passung` only derives rules for what none of these sources
cover.
...
- Roughly how far back does history go, and how many distinct authors
  ...
  maturity signal in `/passung-canonize` Step 2 — report that
...
domain? If so, note it — `/passung-align` must not silently reach
