...
Verify that `/passung-align` on `[dimension]` (`$2`; or every aligned
...
If `/passung-preflight` Check 3 came back green: run the full test
suite (or the scoped subset covering the aligned modules) on the branch
from `/passung-align`, and compare pass/fail counts and results
...
**structural diff review** — for every file `/passung-align` touched,
...
form. This is the same "docs vs. code" drift concern `/passung-scan`
...
align.md's Write contract. Unlike `/passung-align`'s `units`, omit a
...
module's tests, then an independent **passung-critic** pass
...
clean: list blockers and suggest fixing them in `/passung-align`
