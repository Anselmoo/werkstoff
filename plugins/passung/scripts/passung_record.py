...
usage: passung_record.py event --area AREA --stage STAGE --status closed|halted|opened
                                   [--reason TEXT] [--detail JSON] [--root DIR]
       passung_record.py status --area AREA [--root DIR]
       passung_record.py --selftest

passung's eleven artifacts under analysis/<area>/ are all written by the
...
def _run(root: Path, area: str):
    ...
    return run_record, run_record.open_run("passung", "__".join(rel.parts),
...
def selftest() -> int:
    ...
    if fails:
        ...
        return 1
    print("passung_record selftest passed")
    return 0


def main(argv: list) -> int:
    ap = argparse.ArgumentParser(prog="passung_record.py", description=(__doc__ or "").split("\n\n")[0])
