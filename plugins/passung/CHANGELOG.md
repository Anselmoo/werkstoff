...
All notable changes to the `passung` plugin are documented here.
Releases before the rename were published as `codebase-consistency`; tags under that name
...
- **BREAKING: install identity changed.** `/plugin install codebase-consistency@werkstoff` no longer
  resolves; use `/plugin install passung@werkstoff`. There is no alias mechanism in the
  marketplace, so an existing install must be removed and re-added.
- **BREAKING: skill and command ids renamed** -- `/consistency-*` -> `/passung-*`.
- **BREAKING: script module names changed** -- `scripts/consistency_record.py` -> `scripts/passung_record.py`.
- Published `codebase-consistency-v*` tags and their GitHub Releases are unchanged and stay reachable;
