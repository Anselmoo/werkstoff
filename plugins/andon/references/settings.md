...
| `gap_source` | string | `self-scan` | Where `andon-loop` Phase 2 gets the stage's gaps. `self-scan` (default): andon detects gaps itself (failing tests, unproven wires, TODOs, schema drift). `self-assess-brief`: **ingest mode** — andon reads `self-assess`'s `MODERNIZATION_BRIEF.md` (+ `transform_brief_summary.json`) from `self_assess_output_dir` and drives the loop off its phases and work items instead of self-scanning (see `andon-loop/SKILL.md` Phase 0/2). This is the auto-pilot's fix+validate half: self-assess owns check+plan, andon owns fix+validate. |
| `self_assess_output_dir` | string | `analysis/self-assess` | Only read when `gap_source: self-assess-brief`. Where to find the self-assess brief + sidecars to ingest (matches self-assess's own `output_dir` default). |
