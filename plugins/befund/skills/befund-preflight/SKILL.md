...
Determine whether this repository is ready for befund's analysis skills, and which
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py check-enabled --repo <repo_root> --skill befund-preflight
```
...
resolved settings (including `output_dir`, default `analysis/befund`). Use that `output_dir` for
...
- `befund-ci-topology` is `Not-ready` if `git_remotes_ci`'s status is `fail` (no `.git`
  directory found) -- the skill's own rule refuses to run outside git.
- `befund-ui-audit` is `Ready-with-gaps` (never `Not-ready`) when no UI files are found --
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py validate-artifact --kind preflight_summary --file <path-or-inline-json>
```
...
```json
{
  ...
  "verdicts": {
    "befund-arch-health": "Ready",
    "befund-ci-topology": "Ready",
    "befund-ui-audit": "Ready"
  }
}
```

With a gap -- no `.git` directory found, so `git_remotes_ci` is `fail` and
`befund-ci-topology` drops to `Not-ready` per Step 2's rule, while unrelated skills stay
`Ready`:
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py resolve-output-path --repo <repo_root> --filename PREFLIGHT.md
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py resolve-output-path --repo <repo_root> --filename preflight_summary.json
```
