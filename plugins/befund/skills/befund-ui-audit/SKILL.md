...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py check-enabled --repo <repo_root> --skill befund-ui-audit
```
...
```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py validate-artifact --kind ui_audit_summary --file <path-or-inline-json>
```

The validator rejects any `contrast`-kind finding that does not carry `heuristic: true`.

```
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py resolve-output-path --repo <repo_root> --filename UI_AUDIT.md
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/befund_cli.py resolve-output-path --repo <repo_root> --filename ui_audit_summary.json
```
