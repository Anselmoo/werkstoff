#!/usr/bin/env python3
"""Tests for befund-status's dashboard builder.

Reproduces the Phase 2 benchmark finding (docs/plugin-benchmark-phase2-
results.md, SA-2): befund-stage-map's real, schema-validated
stage_graph.json sidecar was invisible to `befund-status` -- its
`present` map only ever tracked the 7 findings-domain sidecars, so a user
asking "where does befund stand" got no signal that stage-map had
...
Run: python3 plugins/befund/scripts/lib/test_status.py
"""
...
class StatusDashboardTests(unittest.TestCase):
    ...
    def test_all_three_structural_skills_tracked_independently(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertNotIn("befund-transform-brief", structural)
    ...
    def test_findings_and_structural_coexist_without_interference(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            _write(d, "stage_map_summary.json")
            self.assertEqual(
                status.build_present_artifacts(d), {"befund-lint-audit": "lint_audit_summary.json"})
            self.assertEqual(
                status.build_structural_artifacts(d),
                {"befund-stage-map": "stage_map_summary.json"})
            self.assertTrue(status.recommend_transform_brief(d))
