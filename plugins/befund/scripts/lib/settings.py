"""Load and default .claude/befund.local.md's YAML frontmatter.
...
def load_settings(repo):
    """Read .claude/befund.local.md if present, defaulted onto DEFAULTS.

    Absence of the file is not an error -- it means fully-defaulted config.
    """
    settings_path = os.path.join(repo, ".claude", "befund.local.md")
    overrides = {}
...
def require_enabled(settings, skill):
    """Raise SelfAssessError if befund, or this specific skill, is disabled."""
    if not settings.get("enabled", True):
        raise SelfAssessError(
            "befund is disabled (enabled: false) in .claude/befund.local.md."
        )
