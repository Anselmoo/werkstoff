...
lets befund_cli.py's single top-level except SelfAssessError handler
...
def check_transform_mode(settings):
    ...
    if mode != "execute":
        raise SelfAssessError(
            ...
            "befund-transform-execute is not authorized to run "
...
def check_not_keep_phase(decision):
    if decision in KEEP_DECISIONS:
        raise SelfAssessError(
            ...
            "by befund-transform-execute (rule: transform-keep-phase-not-executable)."
        )
...
def check_idiom_fix_mode(settings):
    ...
    if mode != "fix":
        raise SelfAssessError(
            f"idiom_fix.mode is {mode!r}, not 'fix'; befund-idiom-fix is "
...
def check_portfolio_scope(cwd, explicit_dir):
    if not explicit_dir and _is_git_repo(cwd):
        raise SelfAssessError(
            f"{cwd!r} is itself a git repository; befund-portfolio needs "
...
def check_autopilot_fix_approved(settings):
    if not (settings.get("autopilot") or {}).get("fix_approved"):
        raise SelfAssessError(
            "autopilot.fix_approved is not true in .claude/befund.local.md; "
            "befund-autopilot is not authorized to apply fixes "
