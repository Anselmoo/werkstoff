#!/usr/bin/env python3
"""Tests for befund-stage-map's HTML viewer builder.
...
1. The committed demo fixture (`scripts/fixtures/sample_stage_graph.json` +
   ...
   `edgeCount == len(wires)`) and still exhibits the failures befund
   ...
   and befund-arch-health use, so the fixture cannot silently stop being
   a failing example.
...
Run: python3 plugins/befund/scripts/test_build_stage_map_html.py -v
"""
...
class BuildTests(unittest.TestCase):
    ...
    def test_s2_head(self):
        self.assertIn("<title>befund — stage map</title>", self.html)
