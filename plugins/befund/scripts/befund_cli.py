...
def _phase_record(repo, s, phase):
    ...
    This record, analysis/befund/transform-phase-<N>/run.jsonl, survives the
    ...
    # Through the same write guard every other befund write uses: a
    ...
    directory = write_guard.resolve_output_path(
        repo, (s or {}).get("output_dir") or "analysis/befund", run_id)
    try:
        return run_record.open_run("befund", run_id, directory=run_record.Path(directory))
...
def build_parser():
    parser = argparse.ArgumentParser(prog="befund_cli")
    ...
    p.add_argument("--phase", default=None, help="record this transform phase under analysis/befund/transform-phase-<N>/")
