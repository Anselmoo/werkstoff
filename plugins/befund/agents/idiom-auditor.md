...
- **Version-scoped find pass.** befund-code-idiom hands you a detected version per
...
Return a JSON list of findings, each with `category` (`modernization` | `smell`), `kind` (a
short idiom-pattern slug identifying the specific pattern, e.g. `percent-format-to-fstring` or
`bare-except`), `file`, `line`, a short description, and `severityNote` only when genuine
ambiguity exists. `kind` is required on every `modernization` finding, since
`befund-idiom-fix` clusters remediation work by `(file, kind)`. One instance of each
category, with concrete values:

```json
[
  {
    "category": "modernization",
    "kind": "percent-format-to-fstring",
