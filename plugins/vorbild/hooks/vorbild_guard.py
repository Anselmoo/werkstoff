#!/usr/bin/env python3
"""vorbild PreToolUse guard — the two refusals that cannot be left to prose.
...
What it denies — exactly two things
-----------------------------------
...
comes from ``.claude/vorbild.local.md`` frontmatter (``root:``), defaulting to
``.design``. The same file carries ``enforcement: off``.

Escape hatch
------------
``VORBILD_DISABLE_GUARD=1``, named in every deny message. The guard fails CLOSED: an
unexpected exception denies rather than silently dropping enforcement.

Usage:
    vorbild_guard.py            # reads one PreToolUse event as JSON on stdin
Exit: 0 allow, 2 deny (the reason is on stderr and in the stdout JSON).
Calibration: python3 plugins/vorbild/hooks/test_vorbild_guard.py
"""
...
SETTINGS = ".claude/vorbild.local.md"
DEFAULT_ROOT = ".design"
ESCAPE = "VORBILD_DISABLE_GUARD=1"
...
def main() -> int:
    ...
    try:
        if os.environ.get("VORBILD_DISABLE_GUARD") == "1":
            return allow()
        ...
        if not root.is_dir():
            return allow()  # not a vorbild project — say nothing at all
        ...
        if hit is not None:
            target = hit
            return deny(
                f"vorbild: {target} is inside the reference store, which is read-only.\n\n"
                ...
                f"(vorbild guard. Override with {ESCAPE}, or `enforcement: off` in "
                f"{SETTINGS}.)"
            )
        ...
        if gated is not None:
            ...
            if pending:
                return deny(
                    f"vorbild: spread {pending} has no recorded choice, so {target} "
        ...
        return deny(
            f"vorbild guard failed: {type(exc).__name__}: {exc}. Denying rather than "
