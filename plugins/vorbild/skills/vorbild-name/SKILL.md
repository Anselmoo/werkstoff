---
name: vorbild-name
description: "Use to INTERPRET measured Design Cards into a named lexicon: every recurring element gets a name, a purpose, a rule and an anti-rule, written machine-addressably so vorbild-dolmetsch can read it back. Trigger on 'name these elements', 'give every element a purpose and a rule', 'write the lexicon', 'vorbild name'. Requires DECODE.md and refuses without it; a rule with no stated failure case is not emitted at all, and a rule whose purpose names neither a measured property nor a named principle is rejected as a copy."
---
