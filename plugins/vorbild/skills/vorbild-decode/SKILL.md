---
name: vorbild-decode
description: "Use to MEASURE what collected references actually do — type scale as ratios rather than px, spacing as a base unit plus steps, colour as roles before values, radii as a series, motion as duration and easing classes, density — emitting one Design Card per finding with its source, its selector-or-page reference and its reliability grade. Trigger on 'decode these references', 'what do these references actually do', 'measure the type scale', 'vorbild decode'. Measures and cites only; it never names, justifies or interprets — that is vorbild-name — and a card is not accepted until a second agent re-derives it from the cited source."
---
