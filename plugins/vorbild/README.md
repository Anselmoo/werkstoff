...
*Vorbild* is the German word for the thing you work from: the exemplar, the model held
up. This plugin takes the references someone actually admires, measures what they do,
names what recurs, and emits the result as a platform-neutral token file plus a
landscape sketchbook a non-technical decision-maker can approve.
...
- **Not a drift auditor.** It never checks whether code still matches the system.
  vorbild **creates** the system; `codebase-consistency` **guards** it.
- **Not `consistency-canonize`.** That derives a form from *the repository's own
  divergent sites* and feeds an in-place rewrite. vorbild derives from *external
  reference exemplars* and feeds an emit. vorbild never scans a repo for divergence —
...
- **Not `lehre`.** `lehre` enforces a doctrine at write time. vorbild has no opinion
...
```
/plugin install vorbild@werkstoff
```

Or, for a checkout: `claude --plugin-dir plugins/vorbild`.
...
> Triggers `vorbild-preflight`: per-phase Ready / Ready-with-gaps / Not-ready, each
...
> Triggers `vorbild-retrofit`: maps existing declarations onto the taxonomy and proves
...
> Triggers `vorbild-decode`: Design Cards with source, selector-or-page reference, and
...
> Triggers `vorbild-name`: writes `LEXIKON.md`, machine-addressable so `dolmetsch` can
...
> Triggers `vorbild-spread`: divergent idioms in one fixed shell so they stay
...
> Triggers `vorbild-dolmetsch`: translates both ways against `LEXIKON.md`, and says so
...
> Triggers `vorbild-emit`: headless Chrome print-to-pdf honouring `@page`, with the
> approval block that names the decision-maker and the gate criteria.

## Hooks

One `PreToolUse` hook, `hooks/vorbild_guard.py`, `type: "command"`. It is **inert unless
...
![vorbild derivation-health report: a dark table-based report listing six references with
...
```bash
python3 plugins/vorbild/scripts/build_derivation_html.py \
  --data plugins/vorbild/scripts/fixtures/derivation-demo.json --out /tmp/derivation.html
```
...
```bash
python3 plugins/nacharbeit/scripts/nacharbeit_lint.py plugins/vorbild --docs-root docs
python3 test/plugins/lint-frontmatter.py plugins/vorbild
python3 test/plugins/lint-release-wiring.py
python3 plugins/vorbild/hooks/test_vorbild_guard.py
python3 test/plugins/verify-hooks-deny.py plugins/vorbild
claude plugin validate plugins/vorbild --strict
```

## Escape hatch

`VORBILD_DISABLE_GUARD=1` disables the hook entirely. It is named in every deny message
