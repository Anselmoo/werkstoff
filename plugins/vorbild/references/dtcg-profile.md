...
that is exactly what is used — so a formatter that does not know vorbild reads past it
...
4. every token carries `com.werkstoff.vorbild.card`, `reliability` and `rights`;
