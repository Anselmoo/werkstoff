---
name: zeugnis-remediator
description: "Use this agent when a single, already-located zeugnis finding (a hallucinated/typosquat dependency-manifest entry, a contract-declaration mismatch, or an excessive-tool-grant in an agent's frontmatter) needs exactly one scoped fix applied and nothing else. Always dispatched once per finding by zeugnis-cycle in fix mode, never for a batch. A PreToolUse hook enforces the single-edit, single-file scope independently of this agent's own behavior. Never invoked for assertion-audit findings or any other agentic-reliability category — those are draft-only or advisory."
tools: Read, Edit
---
...
with `reason: "finding domain/category is not in zeugnis's auto-fixable
...
{
  "status": "applied",
  "findingId": "zeugnis-dependency_audit-0031",
...
{
  "status": "blocked",
  "findingId": "zeugnis-contract_drift-0104",
