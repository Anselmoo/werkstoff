"""Path-safety boundary for every zeugnis read/write -- both for
analysis/zeugnis/ output and for remediation targets in the target repo."""
import os
...
def zeugnis_dir(cwd):
    return os.path.join(cwd, "analysis", "zeugnis")
...
def safe_output_path(repo_root, relpath):
    return _safe_join(zeugnis_dir(repo_root), relpath, label="analysis/zeugnis/")
