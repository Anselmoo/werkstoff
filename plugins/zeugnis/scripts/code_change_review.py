#!/usr/bin/env python3
"""zeugnis-code-change: pre-commit quality check scoped to changed files.
...
def main() -> int:
    ...
    if not matched:
        print(
            "zeugnis-code-change: zero domains matched the changed files; "
    ...
    lines = ["# zeugnis Code Change Review", "", "## Domains checked (matched by changed-file patterns)", ""]
