#!/usr/bin/env python3
"""zeugnis-status: read-only staleness check plus a single next-action
...
DOMAIN_SKILLS = {
    "dependency_audit": ("zeugnis-dependency-audit", "dependency_audit_summary.json"),
    "assertion_audit": ("zeugnis-assertion-audit", "assertion_audit_summary.json"),
    "contract_drift": ("zeugnis-contract-drift", "contract_drift_summary.json"),
    "agentic_reliability": ("zeugnis-agentic-reliability", "agentic_reliability_summary.json"),
}


def _zeugnis_relpath_mtime(repo_root: str, relpath: str):
...
def build_dashboard(repo_root: str) -> dict:
    ...
    for domain, (skill_id, relpath) in DOMAIN_SKILLS.items():
        mtime = _zeugnis_relpath_mtime(repo_root, relpath)
        summary = None
...
def suggest_next_action(domains: dict, ledger, converged) -> dict:
    ...
    if ledger is not None and converged is False:
        return {"action": "zeugnis-cycle", "reason": "prior cycle has not converged yet"}

    if ledger is None:
        return {"action": "zeugnis-cycle", "reason": "all domains have run at least once; a self-optimization pass has not"}
...
<html><head><meta charset="utf-8"><title>zeugnis status</title>
<style>
...
<body>
<h1>zeugnis findings dashboard</h1>
<table>
