#!/usr/bin/env python3
"""Tests for zeugnis-cycle's constraint-domain picker.
...
Run: python3 plugins/zeugnis/scripts/test_cycle_engine.py
"""
...
class ConstraintDomainSelectionTests(unittest.TestCase):
    ...
    def test_malformed_sidecar_does_not_crash_plan_next_pass(self):
        with Repo() as repo:
            (repo.root / "analysis" / "zeugnis" / "contract_drift_summary.json").write_text("not json")
            plan = plan_next_pass(repo)
        self.assertEqual(plan["domain"], "dependency_audit")
