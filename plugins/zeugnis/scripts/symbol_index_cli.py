...
wrapper the cycle skill points you at" that zeugnis-assertion-audit and
zeugnis-contract-drift's "Shared symbol index" sections already reference by
...
analysis/zeugnis/symbol_index/<invocation_id>.json; any other skill in the
same zeugnis-cycle invocation passing the same invocation_id gets the cached
...
def main() -> int:
    ...
    if args.command == "resolve":
        def build_fn():
            pointer, _reused = build_symbol_index.build_or_reuse(Path(repo_root), "zeugnis", args.no_fts)
            run_dir = Path(repo_root) / "analysis" / "zeugnis" / "runs" / pointer["generation_id"]
