---
name: zeugnis-agentic-reliability
...
1. Determine `repo_root` and the set of skill/agent/workflow files to
   ...
   same rule as `zeugnis-contract-drift` — only `true` on an explicit user
   request.
...
4. Run:
   ...
   `analysis/zeugnis/reports/AGENTIC_RELIABILITY.md` (grouped into four sections,
   one per category, plus a separate trivial-exceptions section) and
   `analysis/zeugnis/agentic_reliability_summary.json`. A rejected finding is
...
5. Report the per-category finding counts to the user. Call out
   ...
   `zeugnis-cycle` in fix mode" since that's the only category
   `zeugnis-remediator` is allowed to touch.
