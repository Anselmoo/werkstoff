---
name: zeugnis-code-change
description: "Use when the user asks for a pre-commit quality check on staged or uncommitted changes, wants a quick zeugnis pass over 'what I just changed' before committing, or asks 'is this diff okay to commit'. Only runs the domains whose file patterns match the changed files, and always produces an advisory verdict that never blocks the commit."
---

Run a fast, changed-files-scoped zeugnis pass across whichever domains are
...
the specific `zeugnis-*-audit` skill instead.
...
4. Re-run the script, now passing every matched domain's findings:
   ...
   This writes `analysis/zeugnis/reports/CODE_CHANGE_REVIEW.md` with one section
