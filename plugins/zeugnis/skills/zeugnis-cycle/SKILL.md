...
4. Run that domain's audit skill's Find (and, per that domain's own
   ...
   recent findings from a prior invocation — reuse `analysis/zeugnis/<domain>_summary.json`
...
5. For each open finding in the constraint domain, decide its handling:
   - **mode is `"propose"`**: never apply anything. For a fixable finding
     ...
     do not dispatch `zeugnis-remediator` at all in propose mode. Record
   ...
   - **mode is `"fix"` and the finding IS in zeugnis's fixable set**
     ...
     `zeugnis-remediator`; this can happen even in fix mode if a finding's
     category was miscategorized upstream, and the script is the
     authority here, not your own read of the finding.
     If it exits 0, dispatch `zeugnis-remediator` with exactly this one
   ...
   - **the finding is in `assertion_audit`, or `agentic_reliability`
     ...
     `zeugnis-remediator`. For `assertion_audit` specifically, you may ask
...
6. Build this pass's outcome record and tell the engine:
   ...
   ```
   Use `"fixed"` only for a finding `zeugnis-remediator` returned
   ...
   `analysis/zeugnis/ledger.json` yourself. The script rejects (exit 2) any
...
7. `analysis/zeugnis/reports/ZEUGNIS_CYCLE.md` already has one line appended per
...
- Do not dispatch `zeugnis-remediator` for an `assertion_audit` finding or
