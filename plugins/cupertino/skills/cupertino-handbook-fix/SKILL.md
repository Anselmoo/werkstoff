...
```
Mode gate: handbook.fix.mode = fix (confirmed)
Findings: 4 mechanical, 2 skipped (design judgment required)

| finding            | file:line              | remediation | verifier verdict |
|---------------------|-------------------------|-------------|-------------------|
| spacing-token-4pt   | src/Card.swift:82       | fixed       | accepted          |
| color-literal-hex   | src/Card.swift:104      | fixed       | rejected          |

Nothing was committed or pushed.
```

When the summary has no `mechanical: true` findings:

```
Mode gate: handbook.fix.mode = fix (confirmed)
Findings: 0 mechanical, 3 skipped (design judgment required)

Nothing to fix — no mechanical findings in this report.
Nothing was committed or pushed.
```
