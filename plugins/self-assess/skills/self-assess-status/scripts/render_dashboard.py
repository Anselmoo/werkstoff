...
def main() -> None:
    ...
    marker = "/*__SELF_ASSESS_DASHBOARD_DATA__*/ null"
    ...
    # findings_dashboard_data.json is derived from UNTRUSTED source (category
    # labels, rule names, and other values ultimately trace back to analyzed
    # repo content). It's injected into a <script> block, and the HTML parser
    # closes <script> on the literal bytes "</script>" regardless of JS
    # string context, so a value containing "x</script><script>..." would
    # execute. json.dumps does NOT escape "<". Escape it (JSON-safe) to kill
    # the breakout.
    data = data.replace("<", "\\u003c").replace(">", "\\u003e").replace("&", "\\u0026")

    open(f"{out_dir}/findings-dashboard.html", "w").write(
        tpl.replace(marker, "/*__SELF_ASSESS_DASHBOARD_DATA__*/ " + data)
    )
