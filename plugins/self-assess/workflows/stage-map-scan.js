export const meta = {
  ...
  whenToUse:
    'Invoked by self-assess-stage-map when the Workflow tool is available. Requires args {repoPath, languages: [<any lowercase language name detected by self-assess-preflight Check 1, e.g. "python", "rust", "typescript", "go", "shell">, ...], houseRules?, skipVerification?}. LANG_BRIEFS below has hand-tuned instructions for the top-20 common languages; any other language still runs via a generic fallback brief. No symbolIndexPath here (unlike the other scan workflows): each finder must run ONE single inline exhaustive command per language to build the full import graph — the shared protocol\'s own "exhaustive sweep" exception, since knowing a file list in advance doesn\'t remove the need to parse every file\'s imports. skipVerification (default false) skips the adversarial wire-verification pass, trading precision for speed. Returns structured stages/wires — the calling skill writes STAGE_MAP.md and renders STAGE_MAP.html from the result.',
  phases: [
